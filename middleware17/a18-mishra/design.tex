...
usage weights in percentage between peer entities at 
each hosting level. 
%
...
with a cache allocation weight percentage of 33 and 67. 
The per-VM ratio is
...
%
Simultaneously, the weights for VM1 and VM2 for both the stores
...
%\revised{
To incorporate comprehensive trickle-down and trickle-up policies,
the SSD store can be used when the memory share of the
container is exhuasted and objects pushed back to the memory
cache on acces or in low-memory pressure periods. 
%
These aspects
of the design require identifying events for transitions of objects
in either direction, selection of objects, quantum of transition etc.
%
...
%}
%\puru{remove next few lines, replace with above. \\
...
%\puru{am not sure we should add these lines. does not add much.
%synergy can be added to related work.}
%\revised{
